...
fn main() {
    let library = pkg_config::probe_library("winpr-tools3")
        .expect("pkg-config could not find winpr3. Try setting PKG_CONFIG_PATH if not installed in system directories");
    ...
    writeln!(&mut header, "#ifndef WINPR_TOOLS_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "#define WINPR_TOOLS_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
    ...
    for path in &library.include_paths {
        ...
        if !spath.contains("winpr-tools3") {
            continue;
        }
    ...
    writeln!(&mut header, "").expect(ferr);
    writeln!(&mut header, "#endif /* WINPR_TOOLS_3_WRAPPER_HEADER */").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
