...
fn main() {
    let library = pkg_config::probe_library("winpr3")
    ...
    writeln!(&mut header, "#ifndef WINPR_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "#define WINPR_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
    ...
    writeln!(&mut header, "").expect(ferr);
    writeln!(&mut header, "#endif /* WINPR_3_WRAPPER_HEADER */").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
