...
fn main() {
    let library = pkg_config::probe_library("uwac0")
        .expect("pkg-config could not find uwac0. Try setting PKG_CONFIG_PATH if not installed in system directories");
    ...
    builder = builder.blacklist_item("FP.*");
    builder = builder.blacklist_type("FP.*");
    builder = builder.blacklist_function("FP.*");

    let header_file = out_path.join("wrapper.h");
    let mut header = File::create(&header_file).expect(ferr);
    writeln!(&mut header, "").expect(ferr);
    writeln!(&mut header, "#ifndef UWAC_0_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "#define UWAC_0_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
    ...
    writeln!(&mut header, "").expect(ferr);
    writeln!(&mut header, "#endif /* UWAC_0_WRAPPER_HEADER */").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
