...
fn main() {
    ...
    writeln!(&mut header, "#ifndef FREERDP_SERVER_SHADOW_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "#define FREERDP_SERVER_SHADOW_3_WRAPPER_HEADER").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
    ...
    writeln!(&mut header, "").expect(ferr);
    writeln!(&mut header, "#endif /* FREERDP_SERVER_SHADOW_3_WRAPPER_HEADER */").expect(ferr);
    writeln!(&mut header, "").expect(ferr);
